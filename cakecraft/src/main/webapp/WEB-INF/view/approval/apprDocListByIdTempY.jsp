...
jQuery(document).ready(function($) {
	...
	var mydata = [ //데이터
		<c:forEach var="adY" items="${apprDocListByIdTempY}">
			{no: "${adY.documentNo}", cd: "${adY.documentNm}", subcd: "${adY.documentSubNm}", regId: "${adY.regId}", title: "${adY.documentTitle}", modDtime: "${adY.modDtime}"},
		</c:forEach>
	];
	...
	$("#list").on("click", ".jqgrow td[aria-describedby='list_no']", function () {
		...
		var no = rowData.no;
		window.location.href = "/cakecraft/approval/modifyApprDoc?documentNo=" + no;
	});
